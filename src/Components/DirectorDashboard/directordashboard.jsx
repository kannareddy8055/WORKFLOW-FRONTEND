...
const DirectorDashboard = () => {
  ...
  useEffect(() => {
        const fetchRequests1 = async () => {
          try {
            const res = await fetch(`https://workflow-backend-5.onrender.com/requests/director/directorapproval`, {
              method:"PUT",
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(`https://workflow-backend-5.onrender.com/requests/director/${selectedStatus.toLowerCase()}`);
