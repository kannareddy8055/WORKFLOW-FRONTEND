...
const ApproverDashboard = () => {
  ...
  useEffect(() => {
      const fetchRequests1 = async () => {
        try {
          const res = await fetch(`https://workflow-backend-5.onrender.com/requests/manager/managerapproval`, {
            method:"PUT",
  ...
  const fetchRequests = async () => {
    try {
      const res = await fetch(`https://workflow-backend-5.onrender.com/requests/manager/${selectedStatus.toLowerCase()}`, {
        method:"GET",
        headers: { "Content-Type": "application/json",
          "Authorization": `Bearer ${Cookies.get('jwtToken')}`
          }
